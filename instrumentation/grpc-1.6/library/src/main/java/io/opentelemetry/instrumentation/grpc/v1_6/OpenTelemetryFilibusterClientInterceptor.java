    ...
    return new ForwardingClientCall<REQUEST, RESPONSE>() {
      ...
      public void sendMessage(REQUEST message) {
        ...
        try (Scope ignored = context.makeCurrent()) {
          OpenTelemetryContextStorage otelContextStorage = (OpenTelemetryContextStorage) contextStorage;

          logger.log(Level.INFO, "CMEIK CONTEXT: " + context.toString());
          logger.log(Level.INFO, "CMEIK Context.current(): " + Context.current());
          logger.log(Level.INFO, "CMEIK contextStorage.getExecutionIndex(): " + contextStorage.getExecutionIndex());
          logger.log(Level.INFO, "CMEIK contextStorage.getExecutionIndex(Context.current()): " + otelContextStorage.getExecutionIndexFromContext(Context.current()));
          logger.log(Level.INFO, "CMEIK SOURCE SERVICE: " + serviceName);
          logger.log(Level.INFO, "CMEIK: sendMessage!");
